...
import {
  framesPerSecond, initialOpacity, initialRadius, maxRadius, FEMALE, URL_DATA,
} from './Constants';
...
export default class GeojsonMapService {
  ...
  static addGeojsonSource(map, isMobile, lang, popupText, coords, geojson, sourcename = Date.now()) {
    ...
    map.on('click', `${sourcename}`, (e) => {
      ...
      const html = `<div class="row"><div class="col-sm"><div class="${popupType}"><p>${name}</p>${gender === FEMALE ? this.getHTMLWikipediaLink(link, popupText) : ''}</div></div></div>`;
    ...
    this.addAnimatedPoint(map, `${sourcename}_point`, `${sourcename}_point`, coords);
  }
